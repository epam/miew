...
describe('utils.deriveDeep()', () => {
  ...
  const complexArray = [num, complexObject, str, simpleObject]
  const obj1 = {
    a: 1,
    b: {
      x: 2,
  ...
  it('sets original object as a prototype', () => {
    expect(Object.getPrototypeOf(utils.deriveDeep(simpleObject))).to.equal(
      simpleObject
    )
    expect(Object.getPrototypeOf(utils.deriveDeep(complexObject))).to.equal(
      complexObject
    )
  })

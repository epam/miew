...
describe('selectors', () => {
  ...
  describe('#parse(str)', () => {
    ...
    before(() => {
      ...
      modifiedSelector = proxyquire('./selectors', {
        '../utils/SelectionParser': { parser }
      })
    })
    ...
    it('returns object with noneSelector in selector field if input string is incorrect selection string', () => {
      expect(
        modifiedSelector.default.parse('incorrectSelString').selector
      ).to.deep.equal(selectors.none())
    })
  ...
  describe('SequenceSelector', () => {
    describe('#includesAtom(atom)', () => {
      ...
      it('excludes atom which sequence number is outside range', () => {
        expect(selectors.sequence(rFrom18To20).includesAtom(atom)).to.equal(
          false
        )
      })
    })
  ...
  describe('AndOperator', () => {
    describe('#includesAtom(atom)', () => {
      ...
      it('gives false for true and false selectors results', () => {
        expect(
          selectors.and(selectors.all(), selectors.none()).includesAtom(atom)
        ).to.equal(false)
      })
      it('gives false for false and true selectors results', () => {
        expect(
          selectors.and(selectors.none(), selectors.all()).includesAtom(atom)
        ).to.equal(false)
      })
      it('gives false for false and false selectors results', () => {
        expect(
          selectors.and(selectors.none(), selectors.none()).includesAtom(atom)
        ).to.equal(false)
      })
    })
  })
  ...
  describe('OrOperator', () => {
    describe('#includesAtom(atom)', () => {
      ...
      it('gives true for true and false selector results', () => {
        expect(
          selectors.or(selectors.all(), selectors.none()).includesAtom(atom)
        ).to.equal(true)
      })
      it('gives true for false and true selector results', () => {
        expect(
          selectors.or(selectors.none(), selectors.all()).includesAtom(atom)
        ).to.equal(true)
      })
      it('gives false for false and false selector results', () => {
        expect(
          selectors.or(selectors.none(), selectors.none()).includesAtom(atom)
        ).to.equal(false)
      })
    })
  })

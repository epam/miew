...
function testPlanes(runMiewAndCheckFn) {
  describe('check effects with clip planes', () => {
    it(
      `render parts of ${protein} with clipped surface modes`,
      runMiewAndCheckFn(runReps, `${protein}_zclip`, {
        load: proteinPath,
        settings: { interpolateViews: false },
        reps: [
          { selector: 'all', mode: 'TU', colorer: 'UN' },
          {
            selector: 'sequence 42',
            mode: ['QS', { zClip: true }],
            colorer: 'SQ'
          },
          {
          ...
          {
            selector: 'sequence 19:20',
            mode: ['SE', { zClip: true }],
            colorer: 'SQ'
          },
          {
            selector: 'sequence 2:3',
            mode: ['CS', { zClip: true }],
            colorer: 'SQ'
          }
        ],
    ...
    it(
      `render clipped ${protein} with fog and colored background`,
      runMiewAndCheckFn(runReps, `${protein}_cl_fg_bg`, {
        load: proteinPath,
        settings: {
    ...
    it(
      `render clipped ${protein} with colored background without fog`,
      runMiewAndCheckFn(runReps, `${protein}_cl_bg`, {
        load: proteinPath,
        settings: {
          draft: { clipPlane: true },
          fog: false,
    ...
    it(
      `render clipped ${protein} with fog and transparent background`,
      runMiewAndCheckFn(runReps, `${protein}_cl_fg_tr`, {
        load: proteinPath,
        settings: {
    ...
    it(
      `render clipped ${protein} with colored fog and background`,
      runMiewAndCheckFn(runReps, `${protein}_cl_cf_bg`, {
        load: proteinPath,
        settings: {

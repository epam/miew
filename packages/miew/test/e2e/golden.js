...
const report = {
  ...
  getActualPath: (id) =>
    path.join(
      ...
      `${_zeroes(report.context.index, 3)}_${id}.png`
    ),
  getDifferencePath: (id) =>
    path.join(
      report.path.mismatch,
      `${_zeroes(report.context.index, 3)}_${id}.diff.png`
    ),
  ...
  add(id, shot, diff) {
    ...
    if (diff) {
      ...
      diff
        .getDiffImage()
        .pack()
        .pipe(fs.createWriteStream(report.getDifferencePath(id)))
    }
    this.data.entries.push({
      id,
      desc: this.context.desc,
      expectedImage: report.getHtmlPath(
        diff ? report.getExpectedPath(id) : report.path.missing
      ),
      actualImage: report.getHtmlPath(report.getActualPath(id)),
      differenceImage: report.getHtmlPath(
        diff ? report.getDifferencePath(id) : report.path.missing
      ),
      mismatch,
      rowClass: !diff
        ? 'skip'
        : mismatch > this.data.threshold
        ? 'fail'
        : 'pass'
    })
  },
...
function _prepareBrowser(width = 1024, height = 768) {
  ...
  return driver
    .executeScript(getPadding)
    .then((pad) =>
      driver
        .manage()
        .window()
        .setRect({ width: width + pad[0], height: height + pad[1] })
    )
    ...
    .then((caps) => {
      ...
      const version =
        caps.get('version') || caps.get('browserVersion') || '(unknown version)'
      const platform =
        caps.get('platform') ||
...
function _prepareServer(cfg) {
  ...
  if (!fs.existsSync(path.join(cfg.localPath, 'index.html'))) {
    throw new URIError(
      `App is not found in ${cfg.localPath}, did you forget to build it?`
    )
  }
  const app = express()
...
function shouldMatch(id, test) {
  ...
  return driver
    .executeScript(() => window.miew && window.miew.screenshot(128))
    .then((dataUrl) => {
      ...
      return _matchAsPromised(shot, report.getExpectedPath(id))
        .then((diff) => {
          ...
          expect(
            Number.parseFloat(diff.misMatchPercentage)
          ).to.be.not.greaterThan(report.data.threshold)
        })
        .catch((err) => {

...
function fbxDownload(fn, complexInfo) {
  const pathToDownloadFile = path.resolve(
    __dirname,
    tmpFolder,
    `${complexInfo.pdbId}.fbx`
  )
...
describe('As a third-party developer, I want to ', function () {
  ...
  before(() => {
    ...
    chromeOptions.setUserPreferences({
      'download.default_directory': tmpDirectory
    })
    ...
    golden.report.path.template = path.resolve(
      __dirname,
      'mismatch/index_fbx.hbs'
    )
    return golden
      .startup(driver, cfg)
      .then((url) => {
  ...
  for (let i = 0, n = testComplexes.length; i < n; i++) {
    it(
      `export fbx from ${testComplexes[i].name} modes`,
      fbxDownload((complex) => {

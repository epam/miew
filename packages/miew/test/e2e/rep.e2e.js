...
describe('As a power user, I want to', function () {
  ...
  before(() => {
    driver = new webdriver.Builder()
      .forBrowser('chrome')
      .setIeOptions(
        new ieDriver.Options()
          .requireWindowFocus(true)
          .enablePersistentHover(false)
      )
      .setChromeOptions(
        new chromeDriver.Options().addArguments(chromeOptionsArguments)
      )
      .build()
  ...
  describe('get lists of properties from Miew', () => {
    ...
    function getPropList(list) {
      ...
      return (
        miew &&
        JSON.stringify(
          miew.constructor[list].all.map((entry) => ({
    ...
    ;[
      ...
      it(
        `retrieve a list of ${listName} including "${sampleId}"`,
        getListFromMiew(getPropList, checkList, listName, sampleId)
      )
    })
  })
  ...
  describe('assign different combinations of modes, colorers and materials i.e.', function () {
    ...
    it(
      `load ${protein} (filler test)`,
      runMiewAndCheck(run, protein, {
        load: proteinPath,
        settings: { interpolateViews: false },
        view
      })
    ...
    const suite = this
    before(() =>
      Promise.all([retrieve.modes, retrieve.colorers]).then(
        ([modes, colorers]) => {
          _.each(modes, (mode) => {
            _.each(colorers, (colorer) => {
              suite.addTest(
                it(
                  `set ${mode.name} mode with ${colorer.name} coloring`,
                  runMiewAndCheck(
                    runRep,
    ...
    // const suite = this;
    before(() =>
      retrieve.materials.then((materials) => {
        _.each(materials, (material) => {
          suite.addTest(
            it(
              `set ${material.name} material`,
              runMiewAndCheck(runRep, `${protein}_QS_EL_${material.id}`, {
                load: proteinPath,
                rep: {
                  id: 1,
                  mode: 'QS',
  ...
  describe('check parameters of properties', () => {
    ...
    it(
      `load ${protein} with an appropriate orientation and scale`,
      runMiewAndCheck(runReps, protein, {
        load: proteinPath
      })
    )

    it(
      `render ${protein} width default colorer paramaters`,
      runMiewAndCheck(runReps, `${protein}_precolors`, {
        load: proteinPath,
        reps: [
    ...
    it(
      `check color parameters for ${protein}`,
      runMiewAndCheck(runReps, `${protein}_colors`, {
        load: proteinPath,
        reps: [
          {
            selector: 'chain AA',
            mode: 'CA',
            colorer: ['HY', { gradient: 'hot' }]
          },
          {
            selector: 'chain AB',
            mode: 'BS',
            colorer: ['OC', { gradient: 'blues' }]
          },
          {
            selector: 'chain AD',
            mode: 'LC',
            colorer: ['TM', { min: -5, max: 15 }]
          },
          {
            selector: 'chain AC',
            mode: 'TU',
            colorer: [
              'CO',
              { color: 0xffff00, baseColor: 0xffc0cb, subset: 'nonpolar' }
            ]
          },
          {
            selector: 'chain AF',
            mode: 'CA',
            colorer: ['MO', { gradient: 'reds' }]
          },
          {
            selector: 'chain AH',
            mode: 'CA',
            colorer: ['UN', { color: 0x008000 }]
          },
          {
            selector: 'chain AI',
            mode: 'CA',
            colorer: ['SQ', { gradient: 'blue-red' }]
          },
          {
            selector: 'chain AJ',
            mode: 'BS',
            colorer: ['EL', { carbon: 0x800080 }]
          }
        ]
      })
    )
    ...
    it(
      `render ${protein} with default mode parameters`,
      runMiewAndCheck(runReps, `${protein}_prepare`, {
        load: proteinPath,
        reps: [
    ...
    it(
      `check mode parameters for ${protein}`,
      runMiewAndCheck(runReps, `${protein}_params`, {
        load: proteinPath,
        reps: [
          {
            selector: 'chain AA',
          ...
          },
          {
            ...
            mode: [
              'CA',
              {
                ...
                tension: 0.5,
                ss: {
                  helix: { width: 2, arrow: 3 },
                  strand: { width: 0.5, arrow: 1 }
                }
              }
            ],
            colorer: 'SS'
          ...
          },
          {
            ...
            mode: [
              'SA',
              { probeRadius: 0.5, wireframe: true, subset: 'elem N' }
            ],
            colorer: 'UN'
          },
          {
    ...
    describe('check combinations of selectors and modes, i. e.', function () {
      it(
        `check selectors for ${protein}`,
        runMiewAndCheck(runReps, `${protein}_selectors`, {
          load: proteinPath,
          reps: [
            {
              selector: 'sequence -1:71 and chain AA',
              mode: 'CA',
              colorer: 'SQ'
            },
            ...
            {
              selector: 'chain AJ and protein or hetatm and not water',
              mode: 'TU',
              colorer: 'SS'
            },
      ...
      const suite = this
      before(() =>
        Promise.all([retrieve.modes]).then(([modes]) => {
          _.each(modes, (mode) => {
            suite.addTest(
              it(
                `check selectors for ${protein} with ${mode.name} mode`,
                runMiewAndCheck(runReps, `${protein}_${mode.id}`, {
                  load: proteinPath,
                  reps: [
                    {
                      selector: 'sequence -1:71 and chain AA',
                    ...
                    {
                      selector: 'serial 1079:1629',
                      mode: mode.id,
                      colorer: 'SQ'
                    },
                    {
                      selector: 'chain A3,A2 and nucleic',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain A5 and pyrimidine',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain A6,A7 and purine',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AG and charged',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AG and polar',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AI and nonpolar',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AD and water',
                      mode: mode.id,
                      colorer: 'EL'
                    },
                    {
                      selector: 'chain AD and elem N',
                      mode: mode.id,
                      colorer: 'EL'
                    },
                    {
                      selector: 'chain AD and name CA',
                      mode: mode.id,
                      colorer: 'EL'
                    },
                    {
                      selector: 'chain AC and residue ALA',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AE and aromatic',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AE and basic',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                      selector: 'chain AE and acidic',
                      mode: mode.id,
                      colorer: 'RT'
                    },
                    {
                    ...
                    {
                      selector: 'chain AF and none',
                      mode: mode.id,
                      colorer: 'EL'
                    },
                    {
                      selector: 'chain AK and altloc " "',
                      mode: mode.id,
                      colorer: 'CH'
                    },
                    {
                      selector: 'chain AH and polarh',
                      mode: mode.id,
                      colorer: 'EL'
                    },
                    {
                      selector: 'chain AH and nonpolarh',
                      mode: mode.id,
                      colorer: 'EL'
                    }
                  ]

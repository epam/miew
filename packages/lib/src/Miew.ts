...
class Miew {
  ...
  #OBB = { center: new THREE.Vector3(), halfSize: new THREE.Vector3() }
  #_bSphereForOneVisual = new THREE.Sphere()
  #_bBoxForOneVisual = new THREE.Box3()
  #_bBox = new THREE.Box3()
  #_invMatrix = new THREE.Matrix4()
  #_points = [
    new THREE.Vector3(),
    new THREE.Vector3(),
    new THREE.Vector3(),
  ...
  #_anaglyphMat = new AnaglyphMaterial()
  #_size = new THREE.Vector2()
  #_outlineMaterial = new OutlineMaterial({ depth: true })
  #pars = {
  ...
  #_center = new THREE.Vector3(0.0, 0.0, 0.0)
  #_scene = new THREE.Scene()
  #_camera = new THREE.OrthographicCamera(-1.0, 1.0, 1.0, -1.0, -500, 1000)
  #_material = new THREE.RawShaderMaterial({
    uniforms: {
  ...
  #_geo = gfxutils.buildDistorionMesh(10, 10, settings.now.debug.stereoBarrel)

  #volumeBFMat = new VolumeMaterial.BackFacePosMaterial()
  #volumeFFMat = new VolumeMaterial.FrontFacePosMaterial()
  #cubeOffsetMat = new THREE.Matrix4().makeTranslation(0.5, 0.5, 0.5)
  #world2colorMat = new THREE.Matrix4()
  #_fxaaMaterial = new FXAAMaterial()
  #_aoMaterial = new AOMaterial()
  #_horBlurMaterial = new AOHorBlurMaterial()
  #_vertBlurMaterial = new AOVertBlurWithBlendMaterial()
    ...
    for (let i = 0; i < this._gfx.scene.children.length; i++) {
      if (this._gfx.scene.children[i].type === 'DirectionalLight') {
        ...
        light.shadow.camera.near =
          this.#direction.length() - this.#OBB.halfSize.z
  ...
  _renderOutline(camera, srcDepthBuffer, srcColorBuffer, targetBuffer) {
    ...
    this.#_outlineMaterial.uniforms.color.value = new THREE.Color(
      settings.now.outline.color
    )
    this.#_outlineMaterial.uniforms.threshold.value =
      settings.now.outline.threshold
    this.#_outlineMaterial.uniforms.thickness.value = new THREE.Vector2(
  ...
  setPivotSubset(selector) {
    ...
    if (this.getSelectionCenter(this.#_center, includesAtom, selector)) {
      this._objectControls.setPivot(this.#_center)
      this.dispatchEvent({ type: 'transform' })
    } else {
  ...
  _renderVolume(volumeVisual, camera, dstBuf, tmpBuf1, tmpBuf2, tmpBuf3) {
    ...
    gfx.scene.overrideMaterial = this.#volumeBFMat
    gfx.renderer.render(gfx.scene, camera)

    gfx.renderer.setRenderTarget(tmpBuf2)
    camera.layers.set(gfxutils.LAYERS.VOLUME)
    gfx.scene.overrideMaterial = this.#volumeFFMat
    gfx.renderer.render(gfx.scene, camera)
    ...
    this.#world2colorMat.copy(mesh.matrixWorld).invert()
  ...
  _performFXAA(srcBuffer, targetBuffer) {
    ...
    this.#_fxaaMaterial.uniforms.srcTex.value = srcBuffer.texture
    this.#_fxaaMaterial.uniforms.srcTexelSize.value.set(
      1.0 / srcBuffer.width,
      1.0 / srcBuffer.height
    )
    this.#_fxaaMaterial.uniforms.bgColor.value.set(settings.now.bg.color)

    if (this.#_fxaaMaterial.bgTransparent !== settings.now.bg.transparent) {
      this.#_fxaaMaterial.setValues({
        bgTransparent: settings.now.bg.transparent
      })
      this.#_fxaaMaterial.needsUpdate = true
    }
    gfx.renderer.renderScreenQuad(this.#_fxaaMaterial)
  }
  ...
  _performAO(
    ...
    this.#_aoMaterial.uniforms.diffuseTexture.value = srcColorBuffer.texture
    this.#_aoMaterial.uniforms.depthTexture.value = srcDepthTexture
    this.#_aoMaterial.uniforms.normalTexture.value = normalBuffer.texture
    this.#_aoMaterial.uniforms.srcTexelSize.value.set(
      1.0 / srcColorBuffer.width,
      1.0 / srcColorBuffer.height
    )
    this.#_aoMaterial.uniforms.camNearFar.value.set(
      gfx.camera.near,
      gfx.camera.far
    )
    this.#_aoMaterial.uniforms.projMatrix.value = gfx.camera.projectionMatrix
    this.#_aoMaterial.uniforms.aspectRatio.value = gfx.camera.aspect
    this.#_aoMaterial.uniforms.tanHalfFOV.value = tanHalfFOV
    gfx.root.matrix.extractScale(this.#_scale)
    this.#_aoMaterial.uniforms.kernelRadius.value =
      settings.now.debug.ssaoKernelRadius * this.#_scale.x
    this.#_aoMaterial.uniforms.depthThreshold.value =
      2.0 * this._getBSphereRadius() // diameter
    this.#_aoMaterial.uniforms.factor.value = settings.now.debug.ssaoFactor
    ...
    gfx.renderer.renderScreenQuad(this.#_aoMaterial)

    this.#_horBlurMaterial.uniforms.aoMap.value = tempBuffer1.texture
    this.#_horBlurMaterial.uniforms.srcTexelSize.value.set(
      1.0 / tempBuffer1.width,
      1.0 / tempBuffer1.height
    )
    this.#_horBlurMaterial.uniforms.depthTexture.value = srcDepthTexture
    gfx.renderer.setRenderTarget(tempBuffer)
    gfx.renderer.renderScreenQuad(this.#_horBlurMaterial)

    this.#_vertBlurMaterial.uniforms.aoMap.value = tempBuffer.texture
    this.#_vertBlurMaterial.uniforms.diffuseTexture.value =
      srcColorBuffer.texture
    this.#_vertBlurMaterial.uniforms.srcTexelSize.value.set(
      1.0 / tempBuffer.width,
      1.0 / tempBuffer.height
    )
    this.#_vertBlurMaterial.uniforms.depthTexture.value = srcDepthTexture
    this.#_vertBlurMaterial.uniforms.projMatrix.value =
      gfx.camera.projectionMatrix
    this.#_vertBlurMaterial.uniforms.aspectRatio.value = gfx.camera.aspect
    this.#_vertBlurMaterial.uniforms.tanHalfFOV.value = tanHalfFOV
    const { fog } = gfx.scene
    if (fog) {
      this.#_vertBlurMaterial.uniforms.fogNearFar.value.set(fog.near, fog.far)
      this.#_vertBlurMaterial.uniforms.fogColor.value.set(
        fog.color.r,
        fog.color.g,
        fog.color.b,
    ...
    if (
      this.#_vertBlurMaterial.useFog !== settings.now.fog ||
      this.#_vertBlurMaterial.fogTransparent !== settings.now.bg.transparent
    ) {
      this.#_vertBlurMaterial.setValues({
        useFog: settings.now.fog,
        fogTransparent: settings.now.bg.transparent
      })
      this.#_vertBlurMaterial.needsUpdate = true
    }
    gfx.renderer.setRenderTarget(targetBuffer)
    gfx.renderer.renderScreenQuad(this.#_vertBlurMaterial)
  }

  resetPivot() {
    this.#boundingBox.makeEmpty()
    this._forEachVisual((visual) => {
      this.#boundingBox.union(visual.getBoundaries().boundingBox)
    })
  ...
  setPivotResidue(residue) {
    ...
    if (residue._controlPoint) {
      this.#center.copy(residue._controlPoint)
    } else {
      let x = 0
      let y = 0
      let z = 0
  ...
  _renderSelection(camera, srcBuffer, targetBuffer) {
    ...
    // apply Sobel filter -- draw outline
    this.#_outlineMaterial.uniforms.srcTex.value = srcBuffer.texture
    this.#_outlineMaterial.uniforms.srcTexSize.value.set(
      srcBuffer.width,
      srcBuffer.height
    )
    ...
    this._forEachComplexVisual((visual) => {
      if (
        visual.getSelectionCenter(
          this.#_centerInVisual,
          includesAtom,
          selector || visual.getSelectionBit()
        )
      ) {
        center.add(this.#_centerInVisual)
        count++
      }
    })
    if (count === 0) {

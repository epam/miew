...
function getModes() {
  ...
  return (
    miew &&
    JSON.stringify(
      miew.constructor.modes.all.map((mode) => ({
...
function testShadows(getList, runMiewAndCheckFn) {
  ...
  describe('preliminary step', () => {
    it(
      'get all miew modes',
      getList(getModes, {}, retrieve, checkList, 'modes', 'BS')
    )
  })
  // test all modes with shadows
  const suite =
    describe('assign all combinations of modes to shadows, i.e.', () => {
      // empty test just running miew. its is needed to make before() run and add test for all modes
      it(
        'load 1crn (filler test)',
        runMiewAndCheckFn(runMiew, '1crn', { load: '/data/1crn.pdb' })
      )
      // test all modes
      before(() =>
        retrieve.modes.then((modes) => {
          _.each(modes, (mode) => {
            suite.addTest(
              it(
                `set ${mode.name} mode with shadows on`,
                runMiewAndCheckFn(runMiewWithReps, `1crn_sha_${mode.id}`, {
                  load: '/data/1crn.pdb',
